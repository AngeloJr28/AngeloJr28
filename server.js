...
// 2. MAIN TRACKING ROUTE
app.post('/track', async (req, res) => {
    try {
        const { message } = req.body;
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash", generationConfig: { responseMimeType: "application/json" } });

        const prompt = `User input: "${message}". Identify if MEAL, WEIGHT, or WORKOUT. 
        Return JSON ONLY:
        ...
        res.status(500).json({ success: false, error: "AI or Database Error" });
    }
});

// 3. STATS & HISTORY ROUTES
app.get('/stats', (req, res) => {
    db.get(`SELECT SUM(protein) as p FROM fitness_logs WHERE date(date) = date('now')`, (err, row) => {
        ...
        db.get(`SELECT weight FROM weight_logs ORDER BY date DESC LIMIT 1`, (err, weightRow) => {
            res.json({ protein, weight: weightRow?.weight || "--" });
        });
    });
});
